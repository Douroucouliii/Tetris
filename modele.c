...
void moveDownPiece(cell** c, PieceConfig *p){
    ...
    for(int i = 0 ; i < p ->num_cells;i++){
        int coord_x = p -> coords[i][0];
        int coord_y = p -> coords[i][1];

        c[coord_x][coord_y].isFull = false;
        c[coord_x][coord_y].c = NOTHING;
    }
    ...
    //On met à jour la position de la piece sur la grille
     for(int i = 0 ; i < p ->num_cells;i++){
        int coord_x = p -> coords[i][0];
        int coord_y = p -> coords[i][1];

        c[coord_x][coord_y].isFull = true;
        c[coord_x][coord_y].c = p->c;
    }
}
...
//A l'aide krol, est ce qu'on ajoute la piece au tableau lesPieces une fois que c'est arrivé tout en bas ou non ? OUI